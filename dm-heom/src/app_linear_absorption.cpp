...
int main(int argc, char* argv[])
{
	...
	try {
		...
		heom::linear_absorption_config heom_config(command_line.heom_config_filename());
		...
		// linear absorption uses a pseudo pathway with groud state for dipole computations
		...
		for (size_t tensor_index = 0; tensor_index < num_dipole_matrices; ++tensor_index) {
			...
			for (int_t i = 0; i < iterations; ++i) {
				...
				// update status
				heom::write_progress(current_step - 1, total_steps, solver_run, solver_runs, "Calculation linear absorption: ", std::cout);
			}
