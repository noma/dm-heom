...
/**
 * left multiplication of each hierarchy auxiliary density matrix with input M
 * NOTE: no restrict keyword because arguments might point to the same buffers
 */
__kernel void hierarchy_arithmetic_mmult_left(
	__global const complex_t* A,    // input: hierarchy
	__global const complex_t* M,    // input: matrix with which to multiply
	__global complex_t* result      // output: hierarchy-sized, result[sigma_id] = M * A[sigma_id]
)
{
	...
	for (int i = 0; i < NUM_STATES; ++i)
	{
		for (int j = 0; j < NUM_STATES; ++j)
		{
			...
			for (int k = 0; k < NUM_STATES; ++k) // result[i,j] = sum(k=1,NUM_STATES) M[i,k] * A[k,j]
			{
				value = cadd(value, cmult(M[i * NUM_STATES + k], A[offset + k * NUM_STATES + j]));
			}
