...
/**
 * Substract two hierarchy buffers element-wisely.
 * NOTE: no restrict keyword because arguments might point to the same buffers
 */
__kernel void hierarchy_arithmetic_subtract(
	__global const complex_t* A,    // input: hierarchy one
	__global const complex_t* B,    // input: hierarchy two
	__global complex_t* result      // output: A - B, not restricted because result can point to A or B
) 
{
